...
from config.defaults import REPORT_TIMELINE_MAX, REPORT_STATS_HISTORY_MAX, REPORT_WAF_INTERACTIONS_MAX, REPORT_HEALTH_HISTORY_MAX
...
class AttackReporter:
    ...
    def __init__(self, target: str, output_dir: str = "reports"):
        ...
        # Data collection
        self.timeline: deque = deque(maxlen=REPORT_TIMELINE_MAX)  # W2.4
        self.stats_history: deque = deque(maxlen=REPORT_STATS_HISTORY_MAX)  # W2.4
        self.current_stats: Dict[str, Any] = {}
        self.methods_used: Dict[str, Dict[str, Any]] = {}
        self.waf_interactions: deque = deque(maxlen=REPORT_WAF_INTERACTIONS_MAX)  # W2.4
        self.server_health_history: deque = deque(maxlen=REPORT_HEALTH_HISTORY_MAX)  # W2.4
        ...
        if event_type == "waf_detected":
            ...
            if method not in self.methods_used:
                self.methods_used[method] = {
                    ...
                    "rate_limited": 0, "avg_rt": 0, "_rt_ema": 0.0, "_rt_samples": 0,
                }
            ...
            if rt > 0:
                # PERF-1: Use EMA instead of unbounded list + O(n) sum()
                # Replaces: m["rt_samples"].append(rt); m["avg_rt"] = sum(...) / len(...)
                m["_rt_samples"] += 1
                if m["_rt_samples"] == 1:
                    m["_rt_ema"] = rt
                else:
                    m["_rt_ema"] = 0.1 * rt + 0.9 * m["_rt_ema"]
                m["avg_rt"] = m["_rt_ema"]
...
<div class="container">
  ...
  <div class="footer">
    ...
    def _build_rps_chart(self) -> str:
        ...
        rps_values = [s.get("rps", s.get("requests_per_second", 0)) for s in self.stats_history]
...
if __name__ == "__main__":
    ...
    if args.input:
        try:
            ...
            logger.error(f"Error loading input: {e}", exc_info=True)
