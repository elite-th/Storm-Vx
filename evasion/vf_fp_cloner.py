...
class BrowserFingerprintCloner:
    ...
    def __init__(self, verify_ssl: bool = True):
        ...
        self._probed_target: Dict | None = None
    ...
    async def probe_target(self, url: str) -> Dict:
        ...
        """
        if not HAS_AIOHTTP:
            logger.error("aiohttp not installed — fingerprint probing unavailable")
            return None
        ...
        if not hostname or parsed.scheme != 'https':
            logger.info(f"[FP-CLONE] Target is not HTTPS, TLS probing skipped")
            return result

        logger.info(f"[FP-CLONE] Probing TLS fingerprint of {hostname}:{port}...")
        ...
        for profile_name, profile in BROWSER_PROFILES.items():
            try:
                ...
                timeout = scanner_timeout(total=DEFAULT_TIMEOUT_SECONDS)  # W2.4

                connector = aiohttp.TCPConnector(ssl=ctx, limit=1)  # W2.4: intentional single-connection probe
                async with aiohttp.ClientSession(timeout=timeout, connector=connector) as session:
                    try:
                        async with session.get(url, allow_redirects=False) as resp:
                            status = resp.status
                            logger.debug(f"{profile['name']}: status {status}")
                        ...
                        logger.warning(f"{profile['name']}: connection error — {e}")
            except (ssl.SSLError, OSError, ValueError) as e:
                logger.warning(f"{profile['name']}: SSL error — {e}")
        ...
        if best_profile:
            ...
            logger.info(f"[FP-CLONE] Best profile: {best_profile['name']} (status {best_status})")
        else:
            ...
            try:
                ...
                logger.warning(f"[FP-CLONE] Raw TLS probe failed: {e}")
    ...
    async def _raw_tls_probe(self, hostname: str, port: int) -> Dict:
        """
        Async TLS probe to extract server's TLS configuration.

        BUG-010 fix: Replaced blocking socket.create_connection() with
        asyncio.open_connection() to avoid blocking the event loop.
        """
        result = {
            "ja3_hash": "unknown",
            "tls_version": "TLSv1.3",
        ...
        try:
            ctx = create_ssl_context(self._verify_ssl)
            ctx.set_alpn_protocols(['h2', 'http/1.1'])

            reader, writer = await asyncio.wait_for(
                asyncio.open_connection(hostname, port, ssl=ctx, server_hostname=hostname),
                timeout=EVASION_FPC_TIMEOUT  # W2.4
            )

            try:
                ssl_object = writer.get_extra_info('ssl_object')
                if ssl_object:
                    proto = ssl_object.version()
                    ...
                    logger.info(f"[FP-CLONE] Server TLS: {proto}, cipher={cipher[0] if cipher else '?'}, alpn={selected_alpn}")
            finally:
                writer.close()
                try:
            ...
            logger.debug(f"[FP-CLONE] TLS probe failed: {e}")

        return result

    def _build_ssl_context(self, profile: Dict) -> ssl.SSLContext:
        """Build SSL context mimicking a browser's TLS fingerprint.

        NOTE: Python's ssl module does not provide control over:
        - TLS extension ordering
        - Extension presence/absence (e.g., GREASE, compress_certificate)
        - Elliptic curve ordering
        - Signature algorithm ordering

        These are the three components that constitute a JA3 hash.
        The cipher_suites and ALPN settings here provide approximate matching
        but will NOT produce an identical JA3 fingerprint. For true JA3
        cloning, use curl-impersonate or a C extension.
        """
        ...
        try:
            ...
            logger.debug(f"ALPN protocol negotiation failed: {e}")

        # Disable certificate verification only when verify_ssl is False
    ...
    def _update_weights(self):
        ...
        logger.debug(f"[FP-CLONE] Weight update — best: {BROWSER_PROFILES[best[0]]['name']} ({best[1]:.2f}), "
              f"worst: {BROWSER_PROFILES[worst[0]]['name']} ({worst[1]:.2f})")
