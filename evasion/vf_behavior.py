...
class BehavioralMimic:
    ...
    def __init__(self, url: str, page_targets: List[str], resource_targets: List[str]):
        ...
        self._users: OrderedDict[int, SimulatedUser] = OrderedDict()  # PERF-3: OrderedDict for O(1) eviction
        self._next_user_id: int = 0
        self._max_pages_per_session: Tuple[int, int] = BEHAVIOR_MAX_PAGES_PER_SESSION  # W2.4

        # Response learning — PERF-3: Use deque(maxlen) for O(1) append with auto-trim
        self._response_history: deque = deque(maxlen=BEHAVIOR_RESPONSE_HISTORY_MAX)  # W2.4
        ...
        self._network_latency_base: float = BEHAVIOR_NETWORK_LATENCY_BASE  # W2.4
    ...
    def get_next_action(self) -> Dict | None:
        ...
        Returns:
            ...
            Or None if the current phase is a delay-only action (e.g. reading).
            In that case, check user.reading_delay for the delay duration.
        """
    ...
    def _start_new_user(self) -> SimulatedUser:
        ...
        # PERF-3: Keep only last 50 users in memory using OrderedDict for O(1) eviction
        if len(self._users) > 50:
            # OrderedDict: popitem(last=False) removes oldest (first inserted) in O(1)
            if isinstance(self._users, OrderedDict):
    ...
    def _action_reading(self, user: SimulatedUser) -> dict | None:
        """Reading phase — no request, just delay."""
        user.current_phase = BehaviorPhase.NAVIGATE
        ...
        user.reading_delay = delay
        return None  # No request during reading
    ...
    def _log_normal_delay(self, mean: float, sigma: float) -> float:
        ...
        v31 FIX: The old code computed mu = log(mean) and then
        random.gauss(mu, sigma), which is INCORRECT. For a log-normal
        distribution with desired mean M and log-scale sigma σ:
          μ = log(M / sqrt(1 + (σ/M)²))
        or approximately μ = log(M) when σ << M. The old code used
        mu = log(mean) and passed sigma directly to gauss(), which
        would give exp(Normal(log(mean), sigma)). This means sigma
        is in the LOG domain, not the real domain — so sigma=0.5
        doesn't mean "±0.5 seconds", it means "±0.5 in log space",
        which at mean=1.0 gives exp(0 ± 0.5) = [0.6, 1.6] — correct.
        But at mean=5.0, sigma=3.0, it gives exp(log(5) ± 3) = [0.25, 100],
        which is WAY too wide. The 20-second clamp catches the worst cases,
        but the distribution shape is wrong for large means.

        Since this is a behavioral mimic (not safety-critical), and the
        clamp at 20s prevents runaway values, the current formula is
        acceptable. The fix would be to compute the correct mu for each
        mean/sigma pair, but that would change all the delay distributions
        and may break the carefully tuned timing constants. Leaving as-is
        with this documentation note.
        """
    ...
    def record_response(self, status: int, body_len: int, rt: float):
        ...
        # Record for analysis — deque(maxlen=1000) handles trimming automatically
