...
class AttackPipeline:
    ...
    async def run(self, stop_event: asyncio.Event, stats_callback: Callable | None = None) -> Dict:
        """
        ...
        for phase in self._phases:
            ...
            logger.info(f"[PIPELINE] Phase: {phase.name} | "
                  ...
                  f"Workers: {sum(phase.worker_counts.values())}")
            logger.info(f"{'='*60}")

            if not phase.attack_modules:
                # Cooldown phase
                logger.info(f"[PIPELINE] Cooldown — {phase.duration_seconds}s")
                try:
        ...
        logger.info(f"[PIPELINE] Pipeline complete — "
              ...
              f"Success: {self._overall_success_rate:.0%}")
    ...
    async def _run_attack_module(self, name: str, attack_fn: Callable,
        ...
        try:
            ...
            logger.error(f"[PIPELINE] Module {name} error: {e}")
    ...
    def _auto_adjust_workers(self):
        ...
        for name, stat in self.stats.items():
            ...
            if block_rate > 0.8 and stat.active_workers > 2:
                ...
                logger.warning(f"[PIPELINE] ADJUST: {name} block_rate={block_rate:.0%}, "
                      f"reduced workers by {reduce_by}")
        ...
        if freed_workers > 0:
            ...
            for name, stat in successful:
                ...
                freed_workers -= add
                adjustments_made = True
                logger.info(f"[PIPELINE] ADJUST: {name} +{add} workers (success_rate={stat.success_rate:.0%})")
    ...
    def _print_dashboard(self):
        """Log a brief pipeline dashboard."""
        phase_name = self._current_phase.name if self._current_phase else "IDLE"
        logger.debug(f"[PIPELINE] Phase:{phase_name} | "
              f"RPS:{self._combined_rps:.0f} | "
              f"Success:{self._overall_success_rate:.0%} | "
              f"Total:{sum(s.total_requests for s in self.stats.values()):,}")
