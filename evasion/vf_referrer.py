...
class ReferrerChainSpoofer:
    ...
    def __init__(self, target_domain: str):
        ...
        self._session_source: ReferrerSource | None = None  # Fixed source for current "session"
        ...
        # Pre-built weighted list for fast sampling — BUG-043: replaced
        # memory-inefficient list-of-100 with random.choices()

    def _pick_source(self) -> ReferrerSource:
        """Pick a source based on realistic distribution."""
        sources = list(ReferrerSource)
        weights = [SOURCE_DISTRIBUTION[s] for s in sources]
