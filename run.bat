...
:: --- PYTHONPATH (tester dir MUST be first so attack modules are found) ---
set "PYTHONPATH=%ROOT_DIR%tester;%ROOT_DIR%;%ROOT_DIR%finder;%ROOT_DIR%tracker;%PYTHONPATH%"

echo   [OK] Python  : %PYTHON%
if defined FINDER_PATH (
    echo   [OK] FINDER  : !FINDER_PATH!
) else (
    echo   [WARN] VF_FINDER.py not found — will run tester directly
)
...
if /i "%~1"=="--test-only" ( set "SKIP_FINDER=1" & shift & goto :parse_args )
set "TARGET_URL=%~1"
shift
goto :parse_args
:done_parsing
...
:: Check if profile already exists from a previous run
if exist "VF_PROFILE.json" (
    echo   [INFO] VF_PROFILE.json already exists from previous run
    set "HAS_PROFILE=1"
    goto :finder_done
)

:: Skip finder if --test-only flag or finder not found
if "!SKIP_FINDER!"=="1" goto :skip_finder
if not defined FINDER_PATH goto :skip_finder
...
if exist "VF_PROFILE.json" (
    ...
    echo.
    echo   [WARN] FINDER did not create profile. Tester will run without profile.
    echo.
    goto :finder_done
)

:skip_finder
echo   [INFO] FINDER skipped — running tester directly
echo.
...
if "!HAS_PROFILE!"=="1" (
    ...
    echo   Target: !TARGET_URL! ^(no profile^)
    echo.
    %PYTHON% "!TESTER_PATH!" "!TARGET_URL!" !TESTER_FLAGS!
)
