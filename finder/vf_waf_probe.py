...
class WAFProber:
    ...
    def __init__(self, url: str, waf_name: str = "", timeout: int = 8, verify_ssl: bool = True):
        """
        ...
        self._ssl = ssl_param(verify_ssl)
    ...
    async def run(self) -> Dict:
        ...
        # v2→v4: Determine if WAF is actually present based on evidence
        total = len(blocked) + len(allowed)
        block_rate = len(blocked) / total if total > 0 else 0

        # v4→v5→v30: Robust WAF detection logic
        # "Unknown (0%)" is NEVER returned — if we tested and found no evidence,
        # report "None" (no WAF). If blocks were detected but WAF type can't
        # be identified, report "Unidentified WAF" with confidence based on
        # block rate.
        # v30: The initial "Unknown" fallback is replaced with "None" since
        # we should never return "Unknown" — it's misleading in dashboards.
        detected_waf_name = self.waf_name or "None"
        has_waf_evidence = False
        if not self.waf_name:
            # Check if any blocked payloads had WAF-specific evidence
            has_waf_body_match = False
            non_trivial_blocks = 0  # Blocks that aren't just 404s for path traversal

            for b in blocked:
                body = b.get("body_snippet", "")
                status = b.get("status_code", 0)
                category = b.get("category", "")

                # Check for WAF-specific body patterns
                for pattern in self.BLOCK_INDICATORS["body_patterns"]:
                    if re.search(pattern, body):
                        has_waf_body_match = True
                        has_waf_evidence = True
                        break

                # A 404 for Path Traversal is NOT a WAF block — just file not found
                # Only count non-trivial blocks (status codes that indicate WAF, not normal 404s)
                if status not in (404,) and category != "Path Traversal":
                    non_trivial_blocks += 1

            # Decision logic:
            # - Very low block rate (<15%) with no WAF body/header evidence → No WAF
            # - Only Path Traversal 404s blocked → No WAF (just normal 404s)
            # - Some WAF body patterns found → WAF detected
            # - v5: Significant non-trivial blocks → Unidentified WAF
            if block_rate < 0.15 and not has_waf_evidence:
                detected_waf_name = "None"
            elif non_trivial_blocks == 0 and not has_waf_evidence:
                # Only trivial blocks (Path Traversal 404s) — no real WAF
                detected_waf_name = "None"
            elif not has_waf_body_match and non_trivial_blocks > 0 and block_rate >= 0.15:
                # v5: We have significant blocks but can't identify the WAF type.
                # Report as "Unidentified WAF" instead of "Unknown (0%)"
                detected_waf_name = "Unidentified WAF"
        ...
        if detected_waf_name == "None":
            ...
            # Named WAF from headers but no direct payload evidence
            confidence = 0.5

        return {
            "waf_name": detected_waf_name,
    ...
    async def _send_payloads(self) -> Tuple[List[Dict], List[Dict]]:
        """Send all payloads in parallel batches and categorize results."""
        blocked = []
        allowed = []
        ...
        timeout_cfg = scanner_timeout(total=self.timeout)
        sem = asyncio.Semaphore(WAF_PROBE_SEMAPHORE)  # W2.4

        async with aiohttp.ClientSession(timeout=timeout_cfg) as session:
            for category, payload_list in PAYLOADS.items():
                print(f"  {C.CY}    Testing {category} ({len(payload_list)} payloads)...{C.RS}")

                async def _send_with_sem(item, cat=category):
                    async with sem:
                        result = await self._send_single_payload(
                            session, item["payload"], item["location"], item["param"]
                        )
                        await asyncio.sleep(0.01)  # v15: Reduced from 0.05s to 0.01s
                        return item, cat, result

                tasks = [_send_with_sem(item) for item in payload_list]
                results = await asyncio.gather(*tasks, return_exceptions=True)

                for r in results:
                    sent += 1
                    if isinstance(r, Exception):
                        print(f"  {C.Y}    [{sent}/{total_payloads}] Error: {type(r).__name__}{C.RS}")
                        continue
                    ...
                    print(
                        ...
                        f"{status_str} {C.W}{cat}{C.RS}: "
                        f"{item['payload'][:40]}... → HTTP {result.get('status_code', 'N/A')}"
                    )
        ...
        try:
            if location == "query":
                ...
                async with session.get(url, ssl=self._ssl, allow_redirects=False) as resp:
                    ...
                    result["blocked"] = self._is_blocked(resp.status, body, dict(resp.headers), location)
                ...
                async with session.post(
                    url, data=data, ssl=self._ssl, allow_redirects=False
                ) as resp:
                    result["status_code"] = resp.status
                    body = await safe_read_text(resp)  # W1.10: bounded read
                    result["body_snippet"] = body[:200]
    ...
    def _is_blocked(self, status_code: int, body: str, headers: Dict[str, str],
                    payload_location: str = "") -> bool:
        """
        Determine if a response indicates the payload was blocked by WAF.

        ArvanCloud-specific: 500 status can also indicate a block.
        v2: A 404 response for path-based payloads (Path Traversal) is NOT a
        WAF block — it just means the requested file doesn't exist on the server.
        """
        # v2: A 404 for a path-based (Path Traversal) payload is NOT a WAF block.
        # Path Traversal payloads request non-existent files like /../../../etc/passwd
        # which naturally return 404. Only consider 404 as WAF block if the response
        # body contains WAF-specific patterns (e.g., "Access Denied by WAF").
        if status_code == 404:
            for pattern in self.BLOCK_INDICATORS["body_patterns"]:
                if re.search(pattern, body):
                    return True
        ...
        # v31 FIX: Guard against None waf_name (defensive — type hint says str
        # but callers could pass None). Without this, self.waf_name.lower()
        # would crash with AttributeError.
        if status_code == 500 and (self.waf_name or "").lower() == "arvancloud":
            return True

        # Status code changed from baseline (excluding 404 which is handled above)
    ...
    def _generate_bypass_hints(
        ...
        # v31 FIX: Guard against None waf_name
        if (self.waf_name or "").lower() == "arvancloud":
